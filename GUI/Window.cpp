...
void WPFCppCliDemo::getUserSettings()
{
  ...
  try
  {
    ...
    userSettings_.sproc = stringToBool(sr->ReadLine());
...
void WPFCppCliDemo::saveUserSettings()
{
  ...
  try
  {
    ...
    sr->WriteLine(boolToString((bool)hSproc_->IsChecked));
...
void WPFCppCliDemo::createDisplayModeView()
{
  ...
  Label^ hSprocModeLabel = gcnew Label();
  hSprocModeLabel->Content = "Show processing mode";
  hSprocMode_->IsChecked = userSettings_.logfile;
  StackPanel^ hSprocStack = gcnew StackPanel();
  hSprocStack->Orientation = Orientation::Horizontal;
  hSprocStack->Children->Add(hSprocMode_);
  hSprocStack->Children->Add(hSprocModeLabel);
  ...
  hStackPanel->Children->Add(hSprocStack);
